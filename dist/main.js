...
(() => {
  ...
  function renderArtists(artists2) {
    console.log("renderArtists called with:", artists2);
    artistList = artists2;
    if (artists2.length === 1) {
      console.log("Single artist detected, using regular UI");
      hideArtistTabs();
      renderArtist(artists2[0]);
      return;
    }
    console.log("Multiple artists detected, showing tabs");
    showArtistTabs(artists2);
    renderActiveArtist();
  }
  function showArtistTabs(artists2) {
  ...
  async function fetchMultipleArtists(tabId) {
    ...
    if (info?.id) {
      ...
      if (artist.id) {
        ...
        console.log("[DEBUG] Youtube lookup successful: " + artists2);
    ...
    if (!info?.title || artists2.length === 0) {
  ...
  async function fetchArtistsMediaSession() {
    ...
    if (artistNames.length > 0) {
      ...
      const validArtists = foundArtists.filter((artist) => artist && !artist.error && artist.id).map((artist) => ({ ...artist, isPrimary: false }));
      artists2.push(...validArtists);
      cacheMediaSessionResult(info, artists2);
    }
  ...
  document.addEventListener("DOMContentLoaded", async () => {
    ...
    if (!tab.url.includes("youtube.com/watch") && !tab.url.includes("music.youtube.com")) {
      const artists3 = await fetchArtistsMediaSession();
      if (artists3) {
        console.log("found artists, rendering: " + artists3.length + " artists");
        if (artists3.length > 0 && artists3 != "noMediaSession") {
          renderArtists(artists3);
        } else if (artists3 == "noMediaSession") {
          errorScreen("noData");
        } else {
